...
def default_categorical(input_shape=(120, 160, 3), roi_crop=(0, 0)):
    ...
    x = img_in
    x = Convolution2D(24, (5,5), strides=(2,2), activation='selu', name="conv2d_1")(x)       # 24 features, 5 pixel x 5 pixel kernel (convolution, feauture) window, 2wx2h stride, selu activation
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='selu', name="conv2d_2")(x)       # 32 features, 5px5p kernel window, 2wx2h stride, selu activatiion
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    if input_shape[0] > 32 :
        x = Convolution2D(64, (5,5), strides=(2,2), activation='selu', name="conv2d_3")(x)       # 64 features, 5px5p kernal window, 2wx2h stride, selu
    else:
        x = Convolution2D(64, (3,3), strides=(1,1), activation='selu', name="conv2d_3")(x)       # 64 features, 5px5p kernal window, 2wx2h stride, selu
    if input_shape[0] > 64 :
        x = Convolution2D(64, (3,3), strides=(2,2), activation='selu', name="conv2d_4")(x)       # 64 features, 3px3p kernal window, 2wx2h stride, selu
    elif input_shape[0] > 32 :
        x = Convolution2D(64, (3,3), strides=(1,1), activation='selu', name="conv2d_4")(x)       # 64 features, 3px3p kernal window, 2wx2h stride, selu
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu', name="conv2d_5")(x)       # 64 features, 3px3p kernal window, 1wx1h stride, selu
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    # Possibly add MaxPooling (will make it less sensitive to position in image).  Camera angle fixed, so may not to be needed

    x = Flatten(name='flattened')(x)                                        # Flatten to 1D (Fully connected)
    x = Dense(100, activation='selu', name="fc_1")(x)                                    # Classify the data into 100 features, make all negatives 0
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Dense(50, activation='selu', name="fc_2")(x)                                     # Classify the data into 50 features, make all negatives 0
...
def default_imu(num_outputs, num_imu_inputs, input_shape):
    ...
    x = img_in
    x = Convolution2D(24, (5,5), strides=(2,2), activation='selu')(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='selu')(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='selu')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu')(x)
    x = Flatten(name='flattened')(x)
    x = Dense(100, activation='selu')(x)
    x = Dropout(.1)(x)
...
def default_bhv(num_outputs, num_bvh_inputs, input_shape):
    ...
    x = Convolution2D(24, (5,5), strides=(2,2), activation='selu')(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='selu')(x)
    x = Convolution2D(64, (5,5), strides=(2,2), activation='selu')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu')(x)
    x = Flatten(name='flattened')(x)
    x = Dense(100, activation='selu')(x)
    ...
    y = bvh_in
    y = Dense(num_bvh_inputs * 2, activation='selu')(y)
    y = Dense(num_bvh_inputs * 2, activation='selu')(y)
    y = Dense(num_bvh_inputs * 2, activation='selu')(y)
...
def default_loc(num_locations, input_shape):
    ...
    x = Convolution2D(64, (5,5), strides=(2,2), activation='selu', name="conv2d_3")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='selu', name="conv2d_4")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='selu', name="conv2d_5")(x)
    x = Dropout(drop)(x)
    x = Flatten(name='flattened')(x)
...
def default_latent(num_outputs, input_shape):
    ...
    x = Convolution2D(32, (5,5), strides=(2,2), activation='selu', name="conv2d_2")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='selu', name="conv2d_3")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (3,3), strides=(1,1), activation='selu', name="conv2d_4")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (3,3), strides=(1,1), activation='selu', name="conv2d_5")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='selu', name="conv2d_6")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='selu', name="conv2d_7")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(10, (1,1), strides=(2,2), activation='selu', name="latent")(x)
