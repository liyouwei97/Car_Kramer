...
def default_categorical(input_shape=(120, 160, 3), roi_crop=(0, 0)):
    ...
    x = img_in
    x = Convolution2D(24, (5,5), strides=(2,2), activation='softsign', name="conv2d_1")(x)       # 24 features, 5 pixel x 5 pixel kernel (convolution, feauture) window, 2wx2h stride, softsign activation
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='softsign', name="conv2d_2")(x)       # 32 features, 5px5p kernel window, 2wx2h stride, softsign activatiion
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    if input_shape[0] > 32 :
        x = Convolution2D(64, (5,5), strides=(2,2), activation='softsign', name="conv2d_3")(x)       # 64 features, 5px5p kernal window, 2wx2h stride, softsign
    else:
        x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign', name="conv2d_3")(x)       # 64 features, 5px5p kernal window, 2wx2h stride, softsign
    if input_shape[0] > 64 :
        x = Convolution2D(64, (3,3), strides=(2,2), activation='softsign', name="conv2d_4")(x)       # 64 features, 3px3p kernal window, 2wx2h stride, softsign
    elif input_shape[0] > 32 :
        x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign', name="conv2d_4")(x)       # 64 features, 3px3p kernal window, 2wx2h stride, softsign
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign', name="conv2d_5")(x)       # 64 features, 3px3p kernal window, 1wx1h stride, softsign
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    # Possibly add MaxPooling (will make it less sensitive to position in image).  Camera angle fixed, so may not to be needed

    x = Flatten(name='flattened')(x)                                        # Flatten to 1D (Fully connected)
    x = Dense(100, activation='softsign', name="fc_1")(x)                                    # Classify the data into 100 features, make all negatives 0
    x = Dropout(drop)(x)                                                      # Randomly drop out (turn off) 10% of the neurons (Prevent overfitting)
    x = Dense(50, activation='softsign', name="fc_2")(x)                                     # Classify the data into 50 features, make all negatives 0
...
def default_imu(num_outputs, num_imu_inputs, input_shape):
    ...
    x = img_in
    x = Convolution2D(24, (5,5), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign')(x)
    x = Flatten(name='flattened')(x)
    x = Dense(100, activation='softsign')(x)
    x = Dropout(.1)(x)
    ...
    y = imu_in
    y = Dense(14, activation='softsign')(y)
    y = Dense(14, activation='softsign')(y)
    y = Dense(14, activation='softsign')(y)
    
    z = concatenate([x, y])
    z = Dense(50, activation='softsign')(z)
    z = Dropout(.1)(z)
    z = Dense(50, activation='softsign')(z)
    z = Dropout(.1)(z)
...
def default_bhv(num_outputs, num_bvh_inputs, input_shape):
    ...
    x = Convolution2D(24, (5,5), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(64, (5,5), strides=(2,2), activation='softsign')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign')(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign')(x)
    x = Flatten(name='flattened')(x)
    x = Dense(100, activation='softsign')(x)
    ...
    y = bvh_in
    y = Dense(num_bvh_inputs * 2, activation='softsign')(y)
    y = Dense(num_bvh_inputs * 2, activation='softsign')(y)
    y = Dense(num_bvh_inputs * 2, activation='softsign')(y)
    
    z = concatenate([x, y])
    z = Dense(100, activation='softsign')(z)
    z = Dropout(.1)(z)
    z = Dense(50, activation='softsign')(z)
    z = Dropout(.1)(z)
...
def default_loc(num_locations, input_shape):
    ...
    x = Convolution2D(64, (5,5), strides=(2,2), activation='softsign', name="conv2d_3")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='softsign', name="conv2d_4")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(1,1), activation='softsign', name="conv2d_5")(x)
    x = Dropout(drop)(x)
    x = Flatten(name='flattened')(x)
    ...
    z = Dense(50, activation='softsign')(x)
    z = Dropout(drop)(z)
...
def default_latent(num_outputs, input_shape):
    ...
    x = Convolution2D(32, (5,5), strides=(2,2), activation='softsign', name="conv2d_2")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (5,5), strides=(2,2), activation='softsign', name="conv2d_3")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (3,3), strides=(1,1), activation='softsign', name="conv2d_4")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(32, (3,3), strides=(1,1), activation='softsign', name="conv2d_5")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='softsign', name="conv2d_6")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(64, (3,3), strides=(2,2), activation='softsign', name="conv2d_7")(x)
    x = Dropout(drop)(x)
    x = Convolution2D(10, (1,1), strides=(2,2), activation='softsign', name="latent")(x)
    ...
    x = Flatten(name='flattened')(x)
    x = Dense(256, activation='softsign')(x)
    x = Dropout(drop)(x)
    x = Dense(100, activation='softsign')(x)
    x = Dropout(drop)(x)
    x = Dense(50, activation='softsign')(x)
